...
@onready var town_name_label = $UI/TownName
...
func _ready() -> void:
	var town = RunData.current_town
	if town == null:
		return
	town_name_label.text = town.town_name
...
func _build_saloon() -> void:
	...
	if RunData.current_town.rumor == "":
		...
		RunData.current_town.rumor = rumors.pick_random()
	var rumor_label = Label.new()
	rumor_label.text = "\"%s\"" % RunData.current_town.rumor
	rumor_label.autowrap_mode = TextServer.AUTOWRAP_WORD
	building_content.add_child(rumor_label)
	
	# Recruit — lock to town
	if RunData.current_town.recruit == null:
		RunData.current_town.recruit = CrewMember.generate()
	var recruit = RunData.current_town.recruit
...
func _on_hire_pressed(recruit: CrewMember, btn: Button, lbl: Label) -> void:
	if RunData.spend_money(recruit.hire_cost):
		...
		RunData.current_town.recruit = null
...
func _build_bank() -> void:
	...
	if RunData.current_town.bank_completed:
		var done_label = Label.new()
		done_label.text = "Already hit this bank. Move on."
		building_content.add_child(done_label)
		return
	
	if RunData.current_town.bank_job == null:
		RunData.current_town.bank_job = JobData.generate(JobData.Type.BANK, _get_difficulty())
	RunData.current_job = RunData.current_town.bank_job
...
func _build_train_station() -> void:
	...
	if RunData.current_town.train_completed:
		var done_label = Label.new()
		done_label.text = "Already hit the train. Move on."
		building_content.add_child(done_label)
		return
	
	if RunData.current_town.train_job == null:
		RunData.current_town.train_job = JobData.generate(JobData.Type.TRAIN, _get_difficulty())
	RunData.current_job = RunData.current_town.train_job
